...
const hasOverlappingOpeningHours = (openingHours: MinimalShop['openingHours']): boolean => {
  const byDay: Record<number, { openAt: string; closeAt: string }[]> = {};

  for (const h of openingHours) {
    if (!byDay[h.day]) byDay[h.day] = [];
    byDay[h.day].push({ openAt: h.openAt, closeAt: h.closeAt });
  }

  for (const dayKey in byDay) {
    const slots = byDay[Number(dayKey)]
      .slice()
      .sort((a, b) => a.openAt.localeCompare(b.openAt));

    for (let i = 0; i < slots.length - 1; i += 1) {
      if (slots[i].closeAt > slots[i + 1].openAt) {
        return true;
      }
    }
  }
...
const ShopForm = () => {
  ...
  const handleClickAddHours = () => {
    setShop({
      ...
      openingHours: [
        ...shop.openingHours,
        { day: 1, openAt: '09:00:00', closeAt: '18:00:00' },
      ],
    });
  };

  const handleClickClearHours = (index: number) => {
    setShop({ ...shop, openingHours: shop.openingHours.filter((_, i) => i !== index) });
  };

  const validate = () => {
    const currentErrors = schema(shop);
    setErrors(currentErrors);
    const hasNoFieldError = Object.values(currentErrors).every((o) => o === '');
    const hasOverlap = hasOverlappingOpeningHours(shop.openingHours);
    if (hasOverlap) {
      setToast({
        severity: 'error',
        message: 'Les horaires se chevauchent pour un même jour',
      });
    }
    return hasNoFieldError && !hasOverlap;
  };
      ...
      <Box sx={{ display: 'block', ml: 'auto', mr: 'auto', width: '80%', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, marginBottom: 3 }}>
          {shop.openingHours.map((openingHour, index) => (
            <Paper
              elevation={2}
              key={`hour-${openingHour.day}-${openingHour.openAt}-${openingHour.closeAt}-${index}`}
